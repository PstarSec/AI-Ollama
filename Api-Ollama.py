...
def print_banner():
    ...
    colored_art = "\n".join(
        [colored(line, random.choice(colors)) for line in ascii_art.split("\n")]
    )

    author_info = f"{Fore.CYAN}{' ' * 50}-- Author: Pstar{Style.RESET_ALL}"
    project_url = (
        f"{Fore.CYAN}{' ' * 50}https://github.com/PstarSec/AI-Ollama{Style.RESET_ALL}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-proxy",
        help="使用代理，例如 -proxy=http://127.0.0.1:8080 或 socks5://127.0.0.1:1080",
    )
    parser.add_argument(
        "-t", type=int, default=6, help="设置请求超时时间，单位：秒，默认 6 秒"
    )
